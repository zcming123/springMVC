 ...
 */
    ...
    // 添加@ResponseBody之后，不再走视图解析器那个流程，而是等同于通过 response 对象直接输出数据
    // @RequestBody 用了这个注解，所以前端需要用 Post 请求，不能用 get 请求。
    // 因为 Post 请求方式才会将参数封装到请求体当中。get 方式，参数是跟在 URL 后面。
    ...
    public String handleRedirect(String name,RedirectAttributes redirectAttributes) {
        //return "redirect:handle01?name=" + name;  // 拼接参数安全性、参数长度都有局限，还是 get 请求
        // addFlashAttribute方法设置了一个flash类型属性，该属性会被暂存到 session中，在跳转到页面之后该属性销毁
